 ...
 * @date 31st Dec, 2022
 */
class Transaction_Items extends CI_Controller{
    ...
    public function __construct(){
        ...
        $this->load->model(['transaction_Item', 'item']);
    }
    ...
    */
    ...
    public function index(){
        ...
        $data['pageContent'] = $this->load->view('transaction_Items/transactions', $transData, TRUE);
        $data['pageTitle'] = "Transaction_Items";
    ...
    */
    ...
    public function latr_(){
        ...
        $totalTransactions = $this->transaction_Item->totalTransactions();
        ...
        $config = $this->genlib->setPaginationConfig($totalTransactions, "transaction_Items/latr_", $limit, ['onclick'=>'return latr_(this.href);']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all transactions from db
        $data['allTransactions'] = $this->transaction_Item->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalTransactions > 0 ? ($start+1) . "-" . ($start + count($data['allTransactions'])) . " of " . $totalTransactions : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        
        $json['transTable'] = $this->load->view('transaction_Items/transtable', $data, TRUE);//get view with populated transactions table
    ...
    */
    ...
    public function nso_()
    {
        try {
            ...
            if ($allIsWell) {
                // Insert info into the database and return transaction's receipt
                ...
                $json['msg'] = $returnedData ? "Transaction successfully processed" :
                    ...
                    . "or contact the technical department for assistance";
                $json['transReceipt'] = $returnedData['transReceipt'];
    
                $json['totalEarnedToday'] = number_format($this->transaction_Item->totalEarnedToday());
    
                // Add into event log
                // Function header: addevent($event, $eventRowIdOrRef, $eventDesc, $eventTable, $staffId) in 'genmod'
                $eventDesc = count($arrOfItemsDetails) . " items totaling $" . number_format($cumAmount, 2)
                    . " with reference number {$returnedData['transRef']} was purchased";
    
                $this->genmod->addevent("New Transaction", $returnedData['transRef'], $eventDesc, 'transactions_item', $this->session->admin_id);
            } else {
            ...
            // Log the error
            error_log("Error in nso_(): " . $e->getMessage());
    ...
    */
    ...
    private function insertTrToDb($arrOfItemsDetails, $_mop, $_at, $cumAmount, $_cd, $vatAmount, $vatPercentage, $discount_amount, $discount_percentage, $cust_name, $cust_phone, $cust_email){
        ...
        while($this->transaction_Item->isRefExist($ref));
        ...
        foreach($arrOfItemsDetails as $get){
             ...
             */
            $transId = $this->transaction_Item->add($itemName, $itemCode, "", $qtySold, $unitPrice, $totalPrice, $cumAmount, $_at, $_cd, 
        ...
        if($this->db->trans_status() === FALSE){
            ...
            $dateInDb = $this->genmod->getTableCol('transactions_item', 'transDate', 'transId', $transId);
    ...
    */
    ...
    private function genTransReceipt($allTransInfo, $cumAmount, $_at, $_cd, $ref, $transDate, $_mop, $vatAmount, $vatPercentage, 
        ...
        $transReceipt = $this->load->view('transaction_Items/transreceipt', $data, TRUE);
    ...
    */
    ...
    public function vtr_(){
        ...
        $transInfo = $this->transaction_Item->getTransInfo($ref);
    ...
    */
    ...
    public function report($from_date, $to_date=''){        
        ...
        $data['allTransactions'] = $this->transaction_Item->getDateRange($from_date, $to_date);
        
        $this->load->view('transaction_Items/transReport', $data);
    }
}