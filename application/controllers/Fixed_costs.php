 ...
 * @date 14 August, 2023
 */
class Fixed_costs extends CI_Controller{
    ...
    public function index(){
        $data['pageContent'] = $this->load->view('fixed_costs/costs', '', TRUE);
        $data['pageTitle'] = "Fixed_costs";
    ...
    */
    ...
    public function lclt(){
        ...
        $totalCosts = $this->db->count_all('fixed_costs');
        ...
        $config = $this->genlib->setPaginationConfig($totalCosts, "fixed_costs/lclt", $limit, ['onclick'=>'return lclt(this.href);']);
        
        $this->pagination->initialize($config);//initialize the library class
        
        //get all Costs from db
        $data['allCosts'] = $this->fixed_cost->getAll($orderBy, $orderFormat, $start, $limit);
        $data['range'] = $totalCosts > 0 ? "Showing " . ($start+1) . "-" . ($start + count($data['allCosts'])) . " of " . $totalCosts : "";
        $data['links'] = $this->pagination->create_links();//page links
        $data['sn'] = $start+1;
        
        $json['costsListTable'] = $this->load->view('fixed_costs/costslisttable', $data, TRUE);//get view with populated costs table
    ...
    */
    ...
    public function add(){
        ...
        try {
            // Validation rules
            $this->form_validation->set_rules('costName', 'Cost Name', ['required', 'trim', 'max_length[30]', 'is_unique[fixed_costs.name]'], ['required' => 'The %s field is required.', 'is_unique' => 'A cost with this name already exists.']);
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $insertedId = $this->fixed_cost->add($costName, $costAmount, $costCategory, set_value('costDescription'), $costCurrency, set_value('costStatus'), set_value('costBalance'), set_value('costPaid'));
    
                // Insert into event log
                $desc = "Addition of {$costName} as a new Cost with Amount '{$costAmount}', Currency '{$costCurrency}' and Category '{$costCategory}' to the  Fixed Costs.";
                $insertedId ? $this->genmod->addevent("Creation of new Cost", $insertedId, $desc, "fixed_costs", $this->session->admin_id) : "";
    ...
    */
    ...
    public function edit(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $oldAmount = $this->genmod->gettablecol('fixed_costs', 'amount', 'id', $costId);
            $oldBalance = $this->genmod->gettablecol('fixed_costs', 'balance', 'id', $costId);
            ...
            //update Cost in db
            $updated = $this->fixed_cost->edit($costId, $costName, $costAmount, $costCategory,$costDescription,$costCurrency,$newBalance);
            ...
            $this->genmod->addevent("Cost Update", $costId, $desc, 'fixed_costs', $this->session->admin_id);
        }
    ...
    public function delete() {
        ...
        if ($costId) {
            try {
                ...
                $cost = $this->fixed_cost->getCostInfo(['id' => $costId], ['name']);
                ...
                // Check if the cost name does not contain "rent" (case-insensitive)
                if (stripos($costName, 'rent') === false) {
                    
                    $this->db->where('id', $costId)->delete('fixed_costs');
                    $json['status'] = 1;

                } else {
                    // Cost contains "salary," inform the user it's a system cost
                    $json['error'] = 'You cannot delete system fixed costs.';
                }
            } catch (Exception $e) {
    ...
    */
    public function payCost(){
        ...
        $costName = $this->genmod->getTableCol('fixed_costs', 'name', 'id', $costId);
        
        try {
            // Attempt to pay the cost
            $paid = $this->fixed_cost->payCost($costId, $paymentAmount);
