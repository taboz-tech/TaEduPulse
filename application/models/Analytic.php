...
class Analytic extends CI_Model{
    ...
    public function getDailyTrans($start=0, $limit=10, $order_by='DATE(transDate)', $order_format='DESC'){
        $this->db->select('count(DISTINCT(ref)) as "tot_trans", SUM(totalAmount) as "tot_earned", DATE(transDate) as transactionDate');
    ...
    public function getTransByDays(){
		if($this->db->platform() == "sqlite3" ){
		  ...
		  $run_q = $this->db->query($q, []);
			...
			$this->db->select('count(DISTINCT(ref)) as "tot_trans", SUM(totalAmount) as "tot_earned", DAYNAME(transDate) as "day"');
    ...
    public function getTransByMonths(){
		if($this->db->platform() == "sqlite3"){
		  ...
		  $run_q = $this->db->query($q, []);
			...
			$this->db->select('count(DISTINCT(ref)) as "tot_trans", SUM(totalAmount) as "tot_earned", MONTHNAME(transDate) as "month"');
			$this->db->order_by('tot_earned', 'DESC');
    ...
    public function getTransByYears(){
		if($this->db->platform() == "sqlite3" ){
		  ...
		  $run_q = $this->db->query($q, []);
			...
			$this->db->select('count(DISTINCT(ref)) as "tot_trans", SUM(totalAmount) as "tot_earned", YEAR(transDate) as "year"');
			$this->db->order_by('tot_earned', 'DESC');
    ...
    */
    ...
    public function totalSalesToday() {
        $query = "SELECT SUM(transactions.totalAmount) as 'totalAmountToday' 
                  FROM transactions 
