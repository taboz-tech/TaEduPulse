...
$(document).ready(function(){
    ...
    $("#costSearch").keyup(function(){
        ...
        if(value){
            $.ajax({
                url: appRoot+"search/fixedCostsearch",
                type: "get",
                data: {v:value},
                success: function(returnedData){
    ...
    $("#costsListTable").on('click', '.delCost', function(e){
        ...
        if(costId){
            ...
            if(confirm){
                ...
                $.ajax({
                    url: appRoot+"fixed_costs/delete",
                    method: "POST",
                    data: {i:costId}
                }).done(function(rd){
                    if(rd.status === 1){
    ...
    $("#processPaymentSubmit").click(function () {
        ...
        if (decision) {
            $.ajax({
                type: "POST",
                url: appRoot + "fixed_costs/payCost",
...
function lclt(url) {
    ...
    $.ajax({
        type: 'get',
        url: url ? url : appRoot + "fixed_costs/lclt/",
        data: {
            orderBy: orderBy,
...
function populateCategoriesSelect(url) {
    $.ajax({
        url: url ? url : appRoot + "fixed_costs/getCategoriesForSelect/",
        type: 'GET',
        dataType: 'json',
...
function populateCurrenciesSelect(url) {
    $.ajax({
        url: url ? url : appRoot + "fixed_costs/getCurrenciesForSelect/",
        type: 'GET',
        dataType: 'json',
        success: function(response) {
            if (response.status === 1) {
